...
> Letzte Aktualisierung: 02.02.2026 | Aktuelle Version: **v0.30.75**
...
| # | Ergebnis | Version |
|---|----------|---------|
| 1 | **Build-Fehler behoben**: `const → let dailyData` in `LongtermTrendsView.tsx` (Fehler: "Assignment to constant variable") | v0.30.75 |
| 2 | **Gemini-Prompt repariert**: KI-Tages- und Nachtberichte nutzen jetzt das `aiContext`-Nutzerprofil aus den Systemeinstellungen → Gemini kennt "Marc", schreibt Fließtext statt JSON-Objekte | v0.30.75 |
| 3 | **JSON-Key-Erkennung erweitert**: Alle Parser in PWA und Admin UI erkennen jetzt die Schlüssel `antwort`, `status_bericht`, `summary`, `analyse`, `text`, `message` → kein roher JSON-Block mehr sichtbar | v0.30.75 |
| 4 | **Sparkline-Tooltip verbessert**: Zeigt jetzt Datum ("Fr 28.2."), Icons (☀️ Aktivität, 🌙 Nacht, 🔍 Status), 45 Zeichen Nachttext, breiteres Design | v0.30.75 |
| 5 | **Aktivitäts-Normalisierung (Zeile 14)**: Sparkline und "Aktivitäts-Belastung" nutzen jetzt einen persönlichen Median (2-Pass) → 100% = durchschnittlicher Tag, max. 200% | v0.30.74 |
| 6 | **Drift-Monitor (Zeile 16)**: Page-Hinkley-Test in Python implementiert, Admin UI zeigt Debug-Kachel "🔬 DRIFT-MONITOR" mit Liniendiagramm und Status-Badge | v0.30.74 |
| 7 | **README_de.md aktualisiert**: AURA MASTER-ROADMAP mit neuer Spalte "UI-Darstellung", alle "GEPLANT"-Zeilen und Vergleichstabellen entfernt | v0.30.74 |
...
- **Sparkline 7 Tage** — Balkenhöhe = personalisiertes Aktivitätslevel, Tooltip mit Datum + Icons
...
| 🟡 Mittel | **Sparkline-Tooltip** auf Mobilgeräten nicht sicher ausgelöst | CSS `:hover` funktioniert auf Touch-Geräten unzuverlässig; evtl. JS-basierter Tooltip nötig |
...
### Option A — Empfohlen: PWA Touch-Tooltip
**Problem**: CSS `:hover`-Tooltips funktionieren auf Mobilgeräten (iPhone/Android) nicht zuverlässig.
**Lösung**: Tooltips auf JavaScript-Basis umstellen — Tap öffnet ein kleines Info-Panel unter dem Balken.
**Aufwand**: ~1 Stunde | **Datei**: `src/lib/pwa_server.js`
