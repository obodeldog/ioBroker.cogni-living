...
async function collectStatusData(adapterRef) {
    const data = {
        ...
        metrics: {
            anomalyLabel: null,   // Tages-Status: Unauffällig / Leicht auffällig / Auffällig
            anomalyScore: null,   // Rohwert 0.0–1.0 (für Tooltip)
            nightQuality: null,   // Letzte Nacht: Text aus geminiNight (KI-Analyse)
    ...
    try {
        ...
        // ── Tages-Status aus Anomalie-Score (ehrlichste personalisierte Metrik) ──
        // Vergleicht heutiges Muster mit dem persönlichen historischen Baseline.
        // 0.0 = exakt wie immer, 1.0 = komplett anderes Verhalten.
        const anomalyState = await adapterRef.getStateAsync('analysis.security.lastScore').catch(() => null);
        if (anomalyState && anomalyState.val !== null && anomalyState.val !== undefined) {
            const score = Number(anomalyState.val);
            data.metrics.anomalyScore = score;
            if (score < 0.3)      data.metrics.anomalyLabel = 'Unauffällig';
            else if (score < 0.6) data.metrics.anomalyLabel = 'Leicht auffällig';
            else                  data.metrics.anomalyLabel = 'Auffällig';
        }

        // ── Nacht-Qualität: direkt aus KI-Analyse (geminiNight) – gleiche Quelle wie Admin ──
        ...
        if (geminiNightState && geminiNightState.val) {
            try {
                // Format z.B. "Leichte Unruhe / Bewegungen im Schlaf." → nehme die erste Phrase
                let text = String(geminiNightState.val)
                    .replace(/^["""„]|[""""]$/g, '')  // Anführungszeichen weg
                    .replace(/\.$/, '')                 // Punkt am Ende weg
                    .trim();
                // Nur erste Phrase (vor " / " oder " - " oder Komma)
                const shortText = text.split(/\s*[\/\-]\s*/)[0].trim();
                data.metrics.nightQuality = shortText.length > 0 ? shortText : null;
            } catch(e) {}
        }
        // Fallback: eigener Algorithmus nur wenn kein geminiNight vorhanden
        ...
        // ── KI-Zusammenfassung (Tages-Report) ──
...
function renderData(d) {
  ...
  // ── Letzte Nacht (aus KI-Analyse, selbe Quelle wie Admin) ──
  ...
  if (nq) {
    ...
    nightEl.style.color = lower.includes('ruhig') && !lower.includes('unruhig') ? '#00e676'
      : lower.includes('leicht') ? '#ffab00'
      : lower.includes('unruhig') || lower.includes('aktiv') || lower.includes('wach') ? '#ff3d3d'
      : '';
    nightEl.style.fontSize = nq.length > 14 ? '13px' : '15px';
  } else {
    nightEl.textContent = '–';
    nightEl.style.color = '';
  }
...
function renderSparkline(entries) {
  ...
  bars.innerHTML = normalized.map((e, i) => {
    ...
    const nightInfo = e.nightText ? \`\\nNacht: \${e.nightText}\` : '';
    const tooltip = hasData
      ? \`\${label}: Aktivität \${act}%, Status \${e.anomalyScore !== null ? (e.anomalyScore < 0.3 ? 'Unauffällig' : e.anomalyScore < 0.6 ? 'Leicht auffällig' : 'Auffällig') : '?'}\${nightInfo}\`
      : \`\${label}: Keine Daten\`;
