...
async function getOrCreateToken(adapterRef) {
    ...
    if (!token || token.length < 10) {
        token = generateToken();
        adapterRef.log.info(`[PWA] Generated new family share token: ${token}`);
        // Token in Adapter-Config speichern (für nächsten Start)
...
async function start(adapterRef) {
    ...
    await new Promise((resolve, reject) => {
        serverInstance.listen(port, '0.0.0.0', () => {
            ...
            adapterRef.log.info(`[PWA] 🔗 Local URL: ${localUrl}`);
