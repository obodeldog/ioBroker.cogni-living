...
/**
 * Liest Raum-Verweildauer für heute aus dem roomStats-State.
 * Gibt Minuten im Bad-Raum zurück (zuverlässiger als Event-Zählung).
 */
async function getBathMinutes(adapterRef) {
    try {
        // analysis.activity.roomStats hat { today: {'EG Bad': 25, ...}, ... }
        const s = await adapterRef.getStateAsync('analysis.activity.roomStats').catch(() => null);
        if (s && s.val) {
            const stats = JSON.parse(s.val);
            const today = stats.today || {};
            let bathMins = 0;
            for (const [room, mins] of Object.entries(today)) {
                if (BATH_KEYWORDS.some(k => room.toLowerCase().includes(k))) {
                    bathMins += (typeof mins === 'number' ? mins : 0);
                }
            }
            return bathMins;
        }
...
/**
 * Liest Fenster-Öffnungen aus dem health-State (Python-Analyse) oder History-Datei.
 */
async function getWindowOpenings(adapterRef) {
    try {
        // Primäre Quelle: letzter Tages-Health-Digest hat windowOpenings
        ...
        if (geminiState && geminiState.val) {
            ...
            if (typeof g.windowOpenings === 'number') return g.windowOpenings;
            if (typeof g.freshAir === 'number') return g.freshAir;
        }

        // Fallback: Heutige History-Datei lesen (wird periodisch gespeichert)
        const histFile = await readTodayHistoryFile(adapterRef);
        if (histFile && typeof histFile.windowOpenings === 'number') return histFile.windowOpenings;
        if (histFile && Array.isArray(histFile.ventilation)) return histFile.ventilation.length;
    } catch(e) {}

    // Letzter Fallback: Ventilation-State
    try {
        const ventState = await adapterRef.getStateAsync('analysis.ventilation.todayOpenings').catch(() => null);
        if (ventState && typeof ventState.val === 'number') return ventState.val;
    } catch(e) {}
...
/**
 * Berechnet Nacht-Qualität. Liest aus der History-Datei oder eventHistory.
 */
async function getNightQuality(adapterRef) {
    try {
        // Aus gestrigem History-File (verlässlichste Quelle)
        const histFile = await readYesterdayHistoryFile(adapterRef);
        if (histFile) {
            // Nacht-Events: Slots 44-47 (22:00-23:30) von gestern + Slots 0-15 (00:00-07:30) heute
            const vec = histFile.todayVector || [];
            if (vec.length >= 48) {
                const nightSlots = [...vec.slice(44, 48)]; // 22:00-23:30 gestern
                // Auch heutige Früh-Slots
                ...
                if (todayVecState && todayVecState.val) {
                    const todayVec = JSON.parse(todayVecState.val);
                    if (Array.isArray(todayVec)) nightSlots.push(...todayVec.slice(0, 16)); // 00:00-07:30
                }
                const totalNight = nightSlots.reduce((a, b) => a + b, 0);
                const maxSlot = Math.max(...nightSlots, 0);
                if (totalNight === 0) return 'Ruhig';
    ...
    // Fallback: Letzte Nacht aus eventHistory
    try {
        const events = adapterRef.eventHistory || [];
        const now = new Date();
        const nightEnd = new Date(now); nightEnd.setHours(8, 0, 0, 0);
        if (now < nightEnd) nightEnd.setDate(nightEnd.getDate() - 1);
        const nightStart = new Date(nightEnd); nightStart.setDate(nightStart.getDate() - 1); nightStart.setHours(22, 0, 0, 0);
        const nightEvents = events.filter(e => {
            const ts = e.ts || 0; return ts >= nightStart.getTime() && ts <= nightEnd.getTime();
        });
        if (nightEvents.length === 0) return null;
        const outsideEvents = nightEvents.filter(e => {
            const room = (e.room || e.location || '').toLowerCase();
            return !SLEEP_KEYWORDS.some(k => room.includes(k));
        });
        if (outsideEvents.length <= 2) return 'Ruhig';
        if (outsideEvents.length <= 8) return 'Leicht unruhig';
        return 'Unruhig';
    } catch(e) {}
    return null;
}
...
/**
 * Liest die gestrige History-Datei.
 */
async function readYesterdayHistoryFile(adapterRef) {
    const d = new Date(); d.setDate(d.getDate() - 1);
    return readHistoryFile(adapterRef, d.toISOString().slice(0, 10));
}

/**
 * Liest die heutige History-Datei (wird periodisch gespeichert).
 */
async function readTodayHistoryFile(adapterRef) {
    return readHistoryFile(adapterRef, new Date().toISOString().slice(0, 10));
}
...
async function collectStatusData(adapterRef) {
    const data = {
        ...
        metrics: {
            activity: null,
            nightQuality: null,  // Letzte Nacht: Ruhig / Leicht unruhig / Unruhig
            bathMins: null,      // Bad-Nutzung heute in Minuten
            freshAir: null,      // Fenster-Öffnungen heute
            gaitSpeed: null      // Gangtempo (Sekunden)
        },
        weekSparkline: [],
        alarms: [],
        aiSummary: '',
    ...
    try {
        ...
        // ── Bad-Nutzung aus roomStats (zuverlässig, nicht aus event buffer) ──
        data.metrics.bathMins = await getBathMinutes(adapterRef);

        // ── Frischluft: Primär aus Python-Analyse, Fallback aus History-Datei ──
        data.metrics.freshAir = await getWindowOpenings(adapterRef);

        // ── Nacht-Qualität: Aus History-Datei oder eventHistory ──
        ...
        if (geminiState && geminiState.val) {
            try {
                const g = JSON.parse(geminiState.val);
                data.aiSummary = (g.summary || g.text || '').replace(/^[Aa]nalyse[^:]*:/i, '').trim();
            } catch(e) {}
        }
...
<div id="content" class="hidden">
  ...
  <div class="metrics-grid">
    ...
    <div class="metric-card">
      ...
      <div class="metric-sub">Schlafqualität</div>
    </div>
    <div class="metric-card">
...
function renderData(d) {
  ...
  nightEl.textContent = nq || '–';
  nightEl.style.color = nq === 'Ruhig' ? '#00e676' : nq === 'Leicht unruhig' ? '#ffab00' : nq === 'Unruhig' ? '#ff3d3d' : '';
  nightEl.style.fontSize = nq ? '15px' : '22px';
...
async function triggerAnalysis() {
  ...
  let pct = 0;
  const steps = [
    [10, 'Sensordaten werden geladen...'],
    [30, 'Bewegungsmuster werden analysiert...'],
    [55, 'Schlaf & Aktivität wird bewertet...'],
    [75, 'KI generiert Zusammenfassung...'],
    [90, 'Wird gespeichert...']
  ];
  let stepIdx = 0;
  const interval = setInterval(() => {
    if (stepIdx < steps.length) {
      pct = steps[stepIdx][0];
      stat.textContent = steps[stepIdx][1];
      bar.style.width = pct + '%';
      stepIdx++;
    }
  }, 1800);
  ...
  try {
    ...
    if (resp.ok) {
      bar.style.width = '100%';
      stat.textContent = '✓ Fertig! Daten werden geladen...';
...
async function handleRequest(req, res, adapterRef, validToken) {
    ...
    if (pathname === '/api/analyze' && req.method === 'POST') {
        try {
            ...
            // Health-Analyse triggern (analog zum Admin-Button)
            await adapterRef.setStateAsync('analysis.training.triggerHealth', { val: true, ack: false });
            // Kurz warten damit Python-Analyse starten kann (nicht blockierend)
            setTimeout(async () => {
                try {
                    await adapterRef.setStateAsync('analysis.training.triggerHealth', { val: false, ack: true });
                } catch(e) {}
            }, 500);
            res.writeHead(200, { 'Content-Type': 'application/json' });
