...
async function collectStatusData(adapterRef) {
    ...
    try {
        ...
        if (geminiNightState && geminiNightState.val) {
            if (geminiNightState.ts) data.nightQualityTs = geminiNightState.ts;
            try {
                // geminiNight kann JSON {"analyse":"..."} oder Plaintext sein
                let rawText = String(geminiNightState.val).trim();
                try {
                    const parsed = JSON.parse(rawText);
                    rawText = parsed.analyse || parsed.summary || parsed.text || parsed.antwort || parsed.status_bericht || parsed.message || rawText;
...
async function loadWeekSparkline(adapterRef) {
    ...
    try {
        ...
        // PASS 1: Rohe Event-Totale und Metadaten sammeln
        const rawDays = [];
        for (let i = 6; i >= 0; i--) {
            const d = new Date();
            ...
            const vec = content ? (content.todayVector || []) : null;
            const rawTotal = vec && Array.isArray(vec) ? vec.reduce((a, b) => a + b, 0) : null;
            rawDays.push({ dateStr, content, rawTotal });
        }

        // PASS 2: Persönlicher Median als 100%-Referenz berechnen
        const validTotals = rawDays.map(d => d.rawTotal).filter(t => t !== null && t > 0);
        ...
        for (const { dateStr, content, rawTotal } of rawDays) {
            if (content) {
                // Normalisiert: 100% = persönlicher Durchschnittstag
                const actPct = rawTotal > 0
                    ? Math.min(200, Math.round((rawTotal / personalMedian) * 100))
                    : 0;
                sparkline.push({
                    date: dateStr,
                    ...
                    nightText: (() => {
                        if (!content.geminiNight) return null;
                        try {
                            let t = content.geminiNight;
                            // JSON parsen (Array oder Objekt)
                            if (typeof t === 'string' && (t.startsWith('[') || t.startsWith('{'))) {
                                try {
                                    let p = JSON.parse(t);
                                    if (Array.isArray(p) && p.length > 0) p = p[0];
                                    t = p.summary || p.text || p.analyse || p.antwort || p.status_bericht || p.message || (typeof p === 'string' ? p : t);
                                } catch(e) { /* kein JSON */ }
                            }
                            // Auf ersten Satz kürzen, Anführungszeichen entfernen
                            return String(t).split(/[.!?]/)[0].replace(/^["„\[\{]/, '').trim();
                        } catch(e) { return null; }
                    })()
                });
            } else {
...
/* Touch-Tooltip Panel (JS-gesteuert, funktioniert auch auf Mobilgeraeten) */
.spark-tooltip {
  display: none;
  position: absolute;
  background: #142014;
...
.spark-tooltip.visible { display: block; }
.sparkline-days {
  display: flex;
  gap: 4px;
  margin-top: 4px;
...
<div id="content" class="hidden">
  ...
  <div class="metrics-grid">
    ...
    <div class="metric-card">
      ...
      <div class="metric-sub" id="metricNightTs">KI-Schlafanalyse</div>
    </div>
    <div class="metric-card">
  ...
  <div id="aiCard" class="ai-card hidden">
    <div class="ai-label">KI Analyse <span id="aiSummaryTs" style="font-size:10px;opacity:0.6;font-weight:normal"></span></div>
...
function renderData(d) {
  ...
  if (d.nightQualityTs) {
    const ntsEl = document.getElementById('metricNightTs');
    if (ntsEl) ntsEl.textContent = 'Stand: ' + new Date(d.nightQualityTs).toLocaleString('de-DE', {day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}) + ' Uhr';
  }
  ...
  if (d.aiSummary) {
    ...
    document.getElementById('aiText').textContent = d.aiSummary.replace(/^[Aa]nalyse[^:]*:/i, '').trim();
      ...
      if (tsEl) tsEl.textContent = '— Stand: ' + new Date(d.aiSummaryTs).toLocaleString('de-DE', {day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}) + ' Uhr';
    }
  }
...
function renderSparkline(entries) {
  ...
  bars.innerHTML = normalized.map((e, i) => {
    ...
    // Datum aus e.date berechnen (z.B. "2026-03-01" → "01.03.")
    const dateStr = e.date ? (() => { const d = new Date(e.date + 'T12:00:00'); return \`\${d.getDate()}.\${d.getMonth()+1}.\`; })() : '';
    const fullLabel = isToday ? 'Heute' : (e.day && dateStr ? \`\${e.day} \${dateStr}\` : label);

    const nightShort = e.nightText ? e.nightText.substring(0, 45) + (e.nightText.length > 45 ? '…' : '') : '';
...
// ── Touch-Tooltip fuer Sparkline (Desktop + Mobil) ───────────────────────────
function initSparkTooltips() {
  const tooltip = document.getElementById('sparkTooltip');
  const barsContainer = document.getElementById('sparklineBars');
  if (!tooltip || !barsContainer) return;

  function showTooltip(bar, e) {
    const tip = bar.getAttribute('data-tip');
    if (!tip) return;
    tooltip.innerHTML = tip.split('\\n').map(function(l) {
      return l ? '<span>' + l.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</span>' : '';
    }).join('<br>');
    const card = bar.closest('.sparkline-card');
    const cardRect = card.getBoundingClientRect();
    const daysEl = document.getElementById('sparklineDays');
    const daysRect = daysEl.getBoundingClientRect();
    tooltip.classList.add('visible');
    const tipW = Math.min(220, cardRect.width - 32);
    tooltip.style.maxWidth = tipW + 'px';
    const barRect = bar.getBoundingClientRect();
    let left = barRect.left - cardRect.left + barRect.width / 2 - tipW / 2;
    left = Math.max(8, Math.min(left, cardRect.width - tipW - 8));
    tooltip.style.left = left + 'px';
    tooltip.style.top = (daysRect.bottom - cardRect.top + 8) + 'px';
    if (e) e.stopPropagation();
  }

  barsContainer.addEventListener('click', function(e) {
    const bar = e.target.closest('.sparkline-bar[data-tip]');
    if (!bar) return;
    if (tooltip._lastBar === bar && tooltip.classList.contains('visible')) {
      tooltip.classList.remove('visible');
      tooltip._lastBar = null;
    } else {
      showTooltip(bar, e);
      tooltip._lastBar = bar;
    }
  });

  document.addEventListener('click', function(e) {
    if (!e.target.closest('.sparkline-card')) {
      tooltip.classList.remove('visible');
      tooltip._lastBar = null;
    }
  });
