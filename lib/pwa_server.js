...
// ─── HELPERS FÜR EVENTHISTORY-AUSWERTUNG ─────────────────────────────────────

const BATH_KEYWORDS    = ['bad', 'wc', 'toilet', 'bath', 'hygiene', 'klo'];
const WINDOW_KEYWORDS  = ['fenster', 'window', 'tür', 'door', 'balkon', 'lüft'];
const SLEEP_KEYWORDS   = ['schlaf', 'bedroom', 'bed', 'schlafen'];

/**
 * Zählt Bad-Besuche aus der eventHistory.
 * Ein "Besuch" = mindestens 1 Event in einem Bad-Raum, nach ≥5 Min Pause gewertet.
 */
function countBathVisits(events, todayStart) {
    const todayEvents = events.filter(e => (e.ts || 0) >= todayStart);
    let visits = 0;
    let lastVisitTime = 0;
    let lastRoom = null;
    for (const ev of todayEvents) {
        const room = (ev.room || ev.location || '').toLowerCase();
        if (BATH_KEYWORDS.some(k => room.includes(k))) {
            if (room !== lastRoom || (ev.ts - lastVisitTime) > 5 * 60000) {
                visits++;
                lastVisitTime = ev.ts;
                lastRoom = room;
            }
...
/**
 * Zählt Fenster-/Türöffnungen aus der eventHistory.
 * Erkennt Fenster-Sensoren über Schlüsselwörter im Sensor-Namen oder Raum.
 */
function countWindowOpenings(events, todayStart) {
    const todayEvents = events.filter(e => (e.ts || 0) >= todayStart);
    let count = 0;
    for (const ev of todayEvents) {
        const name = (ev.name || ev.id || ev.deviceName || '').toLowerCase();
        const room = (ev.room || ev.location || '').toLowerCase();
        if (WINDOW_KEYWORDS.some(k => name.includes(k) || room.includes(k))) {
            // Nur "öffnen"-Events zählen (Wert true/1 oder Kontakt öffnet = false bei manchen)
            const val = ev.value;
            if (val === true || val === 1 || val === 'true' || val === 'open') count++;
            else if (val === undefined || val === null) count++; // unbekannt → zählen
        }
    }
    return count;
}
...
/**
 * Berechnet die Nacht-Qualität aus der vergangenen Nacht (22:00-08:00).
 * "Außerhalb-Events" = Events die nicht im Schlafzimmer waren.
 */
function calcNightQuality(events) {
    const now = new Date();
    ...
    // Falls es noch nicht 8 Uhr ist: die aktuelle Nacht (heutige 22:00 bis jetzt)
    if (now < nightEnd) {
        nightEnd.setTime(now.getTime());
        nightStart.setTime(new Date(now).setHours(22, 0, 0, 0));
        nightStart.setDate(nightStart.getDate() - 1);
    }

    const nightEvents = events.filter(e => {
        const ts = e.ts || 0;
        return ts >= nightStart.getTime() && ts <= nightEnd.getTime();
    });

    if (nightEvents.length === 0) return null;

    // Schlafzimmer-Events (drinbleiben = gut) vs. Außerhalb (aufstehen = unruhig)
    ...
    const pct = outsideEvents.length / Math.max(1, nightEvents.length);
    if (outsideEvents.length === 0)  return 'Ruhig';
    if (outsideEvents.length <= 2)   return 'Ruhig';
    if (outsideEvents.length <= 6)   return 'Leicht unruhig';
    if (pct < 0.15)                  return 'Leicht unruhig';
    return 'Unruhig';
}
...
async function collectStatusData(adapterRef) {
    const data = {
        ...
        metrics: {
            ...
            bathVisits: null,      // Bad-Besuche heute
    ...
    try {
        ...
        // ── Bad-Besuche aus eventHistory ──
        data.metrics.bathVisits = countBathVisits(events, todayStart);

        // ── Frischluft aus eventHistory ──
        data.metrics.freshAir = countWindowOpenings(events, todayStart);

        // ── Nacht-Qualität aus eventHistory ──
        data.metrics.nightQuality = calcNightQuality(events);
...
function renderData(d) {
  ...
  document.getElementById('metricBath').textContent = d.metrics.bathVisits !== null && d.metrics.bathVisits > 0 ? d.metrics.bathVisits + 'x' : d.metrics.bathVisits === 0 ? '0x' : '–';
  document.getElementById('metricAir').textContent = d.metrics.freshAir !== null && d.metrics.freshAir >= 0 ? d.metrics.freshAir + 'x' : '–';
