...
class CogniLiving extends utils.Adapter {
    ...
    async startSystem() {
        ...
        // Phase 2: Krankheits-Risiko-Scores
        ...
        const diseaseProfiles = ['fallRisk', 'dementia', 'frailty', 'depression', 'diabetes2', 'sleepDisorder', 'cardiovascular', 'parkinson', 'copd', 'socialIsolation', 'epilepsy', 'diabetes1', 'longCovid', 'bipolar'];
        for (const profile of diseaseProfiles) {
            await this.setObjectNotExistsAsync(`analysis.health.disease.${profile}`, { type: 'state', common: { name: `Disease Risk: ${profile}`, type: 'number', role: 'value', unit: '%', read: true, write: false, def: 0 }, native: {} });
        }
                ...
                try {
                    ...
                    if (this.isProVersion) {
                        ...
                        if (s && s.val) {
                            ...
                            // Phase 2: Krankheits-Risiko-Scores berechnen
                            // Immer alle 3 implementierten Profile berechnen (unabhaengig vom UI-Toggle-Status)
                            // So sind Scores sofort verfuegbar wenn der Nutzer ein Profil aktiviert
                            const PHASE2_PROFILES = ['fallRisk', 'dementia', 'frailty'];
                            if (digests.length >= 5) {
                                const enabledProfiles = PHASE2_PROFILES;
                                this.log.info(`🏥 Computing disease scores for ${enabledProfiles.length} profiles (${digests.length} days)`);
                                pythonBridge.send(this, 'ANALYZE_DISEASE_SCORES', { digests, enabledProfiles }, (result) => {
                                    if (!result || !result.payload) return;
                                    const scores = result.payload;
                                    if (scores.error) {
                                        this.log.warn(`Disease scores error: ${scores.error}`);
                                        return;
                                    }
                                    this.setStateAsync('analysis.health.disease.scores', { val: JSON.stringify(scores), ack: true }).catch(() => {});
                                    for (const [profile, data] of Object.entries(scores)) {
                                        if (data && data.score !== null && data.score !== undefined) {
                                            this.setStateAsync(`analysis.health.disease.${profile}`, { val: data.score, ack: true }).catch(() => {});
                                        }
                                    }
                                    this.log.info(`✅ Disease scores stored: ${Object.keys(scores).map(k => `${k}=${scores[k]?.score ?? 'n/a'}`).join(', ')}`);
                                });
                            }
                        }
                    }
