...
function setupBriefingScheduler(adapter, callbackFn) {
    try {
        if (adapter.briefingJob) { clearInterval(adapter.briefingJob); adapter.briefingJob = null; }
        if (!adapter.config.briefingEnabled) return;

        const timeParts = (adapter.config.briefingTime || "08:00").split(":");
        const targetHour = parseInt(timeParts[0]) || 8;
        const targetMinute = parseInt(timeParts[1]) || 0;
        let firedToday = false;
        let lastFiredDate = '';

        // Jede Minute prüfen: Ist es die konfigurierte Lokalzeit?
        adapter.briefingJob = setInterval(() => {
            const now = new Date();
            const h = now.getHours();
            const m = now.getMinutes();
            const today = now.toISOString().split('T')[0];

            if (h === targetHour && m === targetMinute && today !== lastFiredDate) {
                lastFiredDate = today;
                adapter.log.info(`🌅 BRIEFING FIRED! Zeit: ${h}:${m < 10 ? '0' : ''}${m} Uhr (konfiguriert: ${targetHour}:${targetMinute < 10 ? '0' : ''}${targetMinute})`);
                callbackFn();
            }
        }, 60 * 1000); // jede Minute

        adapter.log.info(`🌅 Briefing geplant für ${targetHour}:${targetMinute < 10 ? '0' : ''}${targetMinute} Uhr (lokale Systemzeit, Interval-Methode)`);
