...
// Wichtig: Wir sind in /lib, also ist das Adapter-Root ein Level höher ('..')
...
function stopPythonService(adapter) {
    if (adapter.pythonProcess) {
        try { adapter.pythonProcess.kill(); } catch (e) { }
...
function installRequirementsInVenv(adapter, reqPath, callback) {
    ...
    exec(cmd, { timeout: 1800000 }, (err, stdout, stderr) => {
        ...
        if (err) {
            ...
            try { fs.rmSync(VENV_PATH, { recursive: true, force: true }); } catch (e) { }
        }
        else {
...
function installPythonDependencies(adapter, callback) {
    if (adapter.dependencyInstallInProgress) return;
    ...
    if (fs.existsSync(VENV_PATH) && !fs.existsSync(PIP_EXE)) {
        ...
        try { fs.rmSync(VENV_PATH, { recursive: true, force: true }); } catch (e) { adapter.log.error(`Delete failed: ${e.message}`); }
    }
