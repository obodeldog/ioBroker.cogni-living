...
async function sendMorningBriefing(adapter) {
    adapter.log.info('🌅 Starting Morning Briefing (data-driven)...');
    try {
        ...
        // Gestrige Nacht-Daten laden (Briefing läuft um 08:00 → gestern = letzte Nacht)
        ...
        // ── Schlafzeit ──
        let sleepStr = '';
        if (night.sleepWindowStart && night.sleepWindowEnd) {
            const fmt = m => `${String(Math.floor(m/60)).padStart(2,'0')}:${String(m%60).padStart(2,'0')}`;
            const dur = night.sleepWindowDuration ? ` (${Math.round(night.sleepWindowDuration/60)}h)` : '';
            sleepStr = `😴 Schlaf: ${fmt(night.sleepWindowStart)}–${fmt(night.sleepWindowEnd)}${dur}`;
        }
        ...
        if (vibCount !== undefined && vibCount !== null) {
            const label = vibCount === 0 ? 'Ruhige Nacht' : vibCount <= 3 ? 'leicht unruhig' : 'unruhig';
            unruheStr = `🛏️ Nacht: ${label} (${vibCount} Bewegungen)`;
        } else if (motCount !== undefined && motCount !== null) {
            const label = motCount === 0 ? 'Ruhige Nacht' : motCount <= 3 ? 'leicht unruhig' : 'unruhig';
            unruheStr = `🛏️ Nacht: ${label} (${motCount} Ereignisse)`;
        }

        // ── Nykturie ──
        let nocturiaStr = '';
        if (night.nocturiaCount !== undefined && night.nocturiaCount > 0) {
            nocturiaStr = `🚿 Aufgestanden: ${night.nocturiaCount}× in der Nacht`;
        }
        ...
        const lines = ['Guten Morgen!'];
        if (sleepStr)    lines.push(sleepStr);
        if (unruheStr)   lines.push(unruheStr);
        if (nocturiaStr) lines.push(nocturiaStr);
        if (!sleepStr && !unruheStr && !nocturiaStr) lines.push('Noch keine Nacht-Daten verfügbar.');
