...
async function sendMorningBriefing(adapter) {
    adapter.log.info('🌅 Starting Morning Briefing (Gemini + echte Nacht-Daten)...');
    try {
        ...
        // ── Gestrige Nacht-Daten laden ──
        // Nacht endet am HEUTIGEN Tag -> sleepWindowEnd steht in der heute-Datei
        const todayStr = new Date().toISOString().slice(0, 10);
        const yesterdayStr = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
        const _fToday = _path.join(dataDir, todayStr + '.json');
        const _fYesterday = _path.join(dataDir, yesterdayStr + '.json');
        const filePath = (dataDir && _fs.existsSync(_fToday)) ? _fToday : _fYesterday;
        let night = {};
        if (dataDir && _fs.existsSync(filePath)) {
            try { night = JSON.parse(_fs.readFileSync(filePath, 'utf8')); } catch(e) {}
        }
        // Fallback: heute-Datei geladen aber kein Schlafstart enthalten -> gestern versuchen
        if (!night.sleepWindowStart && filePath === _fToday && dataDir && _fs.existsSync(_fYesterday)) {
            try { night = JSON.parse(_fs.readFileSync(_fYesterday, 'utf8')); } catch(e) {}
        }

        // ── Schlafzeit formatieren ──
        const fmt = m => `${String(Math.floor(m/60)).padStart(2,'0')}:${String(m%60).padStart(2,'0')}`;
        const fmtMs = ms => { const d = new Date(ms); return String(d.getHours()).padStart(2,'0') + ':' + String(d.getMinutes()).padStart(2,'0'); };
        const sleepStart = night.sleepWindowStart ? fmtMs(night.sleepWindowStart) : null;
        const sleepEnd   = night.sleepWindowEnd   ? fmtMs(night.sleepWindowEnd)   : null;
        const sleepDurH  = night.sleepWindowDuration ? Math.round(night.sleepWindowDuration / 60) : null;

        // ── Datenpunkte als Text für Gemini-Prompt ──
        const dataLines = [];
        if (sleepStart && sleepEnd) dataLines.push(`- Schlafzeit: ${sleepStart} bis ${sleepEnd}${sleepDurH ? ' (' + sleepDurH + ' Stunden)' : ''}`);
        const vibCount = night.nightVibrationCount;
        const motCount = night.nightMotionCount;
        if (vibCount !== undefined && vibCount !== null) dataLines.push(`- Nacht-Bewegungen (Vibration): ${vibCount}`);
        else if (motCount !== undefined && motCount !== null) dataLines.push(`- Nacht-Ereignisse (Bewegung): ${motCount}`);
        if (night.nocturiaCount !== undefined && night.nocturiaCount > 0) dataLines.push(`- Aufgestanden in der Nacht: ${night.nocturiaCount}×`);
        if (night.bedPresenceMinutes) dataLines.push(`- Zeit im Bett: ${night.bedPresenceMinutes} Minuten`);

        // ── Wochentrend-Sparkline ──
        let sparkline = '';
        try {
            const weekVals = [];
            for (let i = 6; i >= 0; i--) {
                ...
                const fp = _path.join(dataDir, `${ds}.json`);
                if (_fs.existsSync(fp)) {
                    const c = JSON.parse(_fs.readFileSync(fp, 'utf8'));
                    const vec = c.todayVector || [];
        ...
        // ── Gemini: natürlichsprachiger Text auf Basis echter Daten ──
        let text = 'Guten Morgen! Starte gut in den Tag.';
        if (adapter.geminiModel && dataLines.length > 0) {
            try {
                updateApiTimestamp();
                const dataContext = dataLines.join('\n');
                const prompt = `Du bist ein fürsorglicher Gesundheitsassistent namens NUUKANNI. Hier sind die Schlaf- und Nacht-Daten von gestern Nacht:\n${dataContext}\n\nBitte schreibe einen freundlichen, persönlichen "Guten Morgen"-Satz (maximal 2 Sätze, auf Deutsch), der diese Daten natürlich einbaut. Keine Emojis, kein JSON, kein Markdown – nur Text.`;
                const result = await adapter.geminiModel.generateContent(prompt);
                text = result.response.text().replace(/^[Aa]nalyse[^:]*:/i, '').trim();
            } catch(e) {
                adapter.log.warn('🌅 Gemini Briefing Error: ' + e.message + ' – Fallback auf Template.');
                // Fallback: Template-Text wenn Gemini fehlschlägt
                const parts = ['Guten Morgen!'];
                if (sleepStart && sleepEnd) parts.push(`Du hast von ${sleepStart} bis ${sleepEnd} geschlafen.`);
                if (vibCount !== undefined && vibCount === 0) parts.push('Es war eine ruhige Nacht.');
                else if (vibCount > 3) parts.push('Die Nacht war etwas unruhig.');
                text = parts.join(' ');
            }
        } else if (dataLines.length === 0) {
            text = 'Guten Morgen! Noch keine Nacht-Daten vorhanden – bitte sicherstellen dass Sensoren konfiguriert sind.';
        }

        const fullMsg = text + sparkline;
        setup.sendNotification(adapter, fullMsg, false, false, '🌅 NUUKANNI: Guten Morgen');
        adapter.log.info(`🌅 Morning Briefing sent (${yesterdayStr}).`);
    } catch(e) {
        adapter.log.error('Morning Briefing Error: ' + e.message);
    }
}
