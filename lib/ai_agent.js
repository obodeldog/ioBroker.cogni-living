...
async function createDailyDigest(adapter, pythonBridge) {
    ...
    try {
        ...
        // --- FIX: NOTIFICATION SENDEN ---
        const msg = `📅 Daily Digest:\n${json.summary}\nAktivität: ${json.activityLevel}`;
        setup.sendNotification(adapter, msg, false, false);
        // --------------------------------
...
async function generateHealthReport(adapter, reportType) {
    ...
    if (reportType === 'NIGHT') {
        ...
        promptTask = "Analysiere diese Events der NACHT (22:00-08:00). War der Schlaf ruhig? Gab es Badgänge? Antwort kurz für den besorgten Sohn (max 2 Sätze).";
    }
    else {
        // Tag = Heute 08:00 bis Jetzt
        const todayEight = new Date(nowObj);
        todayEight.setHours(8, 0, 0, 0);
        startTime = todayEight.getTime();

        promptTask = "Analysiere diese Events des TAGES (08:00-Jetzt). Aktivität? Mahlzeiten? Haustür? Antwort kurz für den besorgten Sohn (max 2 Sätze).";
    }
    ...
    if (relevantEvents.length === 0) {
        const txt = reportType === 'NIGHT' ? "Keine Daten für die Nacht aufgezeichnet (Sensoren ruhig)." : "Bisher keine Aktivität seit 08:00 Uhr verzeichnet.";
    ...
    const prompt = `
    ROLLE: Medizinischer Assistent für AAL (Ambient Assisted Living).
    AUFGABE: ${promptTask}
    EVENTS:\n${eventLog}
    ANTWORT-STIL: Sachlich, beruhigend, präzise. Keine Floskeln.
    `;
