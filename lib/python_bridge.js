...
function handleResult(adapter, data) {
    ...
    for (const [command, callback] of pendingCallbacks.entries()) {
        ...
        if (data.type === expectedResponse || data.type === command + '_RESULT') {
            ...
            callback(data);
            return; // Callback hat Response verarbeitet
        }
    }
    ...
    else if (data.type === 'HEALTH_RESULT') { adapter.setStateAsync('analysis.health.isAnomaly', { val: data.payload.is_anomaly, ack: true }); }
