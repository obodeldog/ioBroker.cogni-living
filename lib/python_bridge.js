...
let lastSecurityAlarmTs = 0; // Cooldown: max 1 Alarm pro Stunde
...
function handleResult(adapter, data) {
    ...
    if (data.type === 'PONG') adapter.log.debug(`🐍 Python Pong`);
        ...
        if (is_anomaly) {
            const now = Date.now();
            const cooldownMs = 60 * 60 * 1000; // 1 Stunde Cooldown zwischen Alarmen
            if (now - lastSecurityAlarmTs < cooldownMs) {
                adapter.log.debug(`Security: Anomalie erkannt (Score: ${anomaly_score.toFixed(2)}), aber Cooldown aktiv (${Math.round((cooldownMs - (now - lastSecurityAlarmTs)) / 60000)} Min bis nächster Alarm)`);
            } else {
                lastSecurityAlarmTs = now;
                const scoreText = anomaly_score < 0.3 ? 'leicht ungewöhnlich' : anomaly_score < 0.6 ? 'auffällig' : 'stark abweichend';
                const time = new Date().toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' });
                let msg = `Aktivitätsmuster um ${time} Uhr ist ${scoreText} (Score: ${(anomaly_score * 100).toFixed(0)}%)`;
                if (explanation && explanation !== 'Unknown' && !explanation.includes('Fallback')) msg += `\nDetails: ${explanation}`;
                adapter.setStateAsync('analysis.isAlert', { val: true, ack: true });
                adapter.setStateAsync('analysis.alertReason', { val: msg, ack: true });
                setup.sendNotification(adapter, `⚠️ AURA: ${msg}`, false, true);
                adapter.log.warn(`Security Alarm: ${msg}`);
            }
        }
    }
    ...
    // --- STANDARD HANDLER ---
        ...
        if (data.payload.anomaly_score !== undefined && data.payload.anomaly_score !== null) {
            const healthScore = data.payload.anomaly_score;
            adapter.setStateAsync('analysis.health.anomalyScore', { val: healthScore, ack: true });
            // Sync: UI liest analysis.security.lastScore für "Diagnose & Vitalität"
            // ANALYZE_SEQUENCE wird derzeit nicht verwendet, daher hier synchronisieren
            adapter.setStateAsync('analysis.security.lastScore', { val: healthScore, ack: true });
        }
    }
