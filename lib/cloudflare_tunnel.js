'use strict';

/**
 * CLOUDFLARE QUICK TUNNEL MANAGER
 * Version: 0.30.61
 * 
 * Startet einen "Quick Tunnel" via cloudflared binary.
 * Kein Cloudflare-Account nötig – generiert eine temporäre .trycloudflare.com URL.
 * 
 * Für eine permanente URL → Named Tunnel mit Cloudflare-Account nötig.
 * Anleitung: https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/
 */

const { spawn, execSync } = require('child_process');
const https = require('https');
const fs = require('fs');
const path = require('path');
const os = require('os');

let tunnelProcess = null;
let currentUrl = null;
let adapter = null;

// ─── DOWNLOAD ─────────────────────────────────────────────────────────────────

function getCloudflaredPath(dataDir) {
    const ext = process.platform === 'win32' ? '.exe' : '';
    return path.join(dataDir, `cloudflared${ext}`);
}

function getDownloadUrl() {
    const platform = process.platform;
    const arch = process.arch;
    
    // GitHub Release URLs für cloudflared
    const base = 'https://github.com/cloudflare/cloudflared/releases/latest/download/';
    
    if (platform === 'linux') {
        if (arch === 'arm64') return base + 'cloudflared-linux-arm64';
        if (arch === 'arm')   return base + 'cloudflared-linux-arm';
        return base + 'cloudflared-linux-amd64';
    }
    if (platform === 'win32') {
        return base + 'cloudflared-windows-amd64.exe';
    }
    if (platform === 'darwin') {
        if (arch === 'arm64') return base + 'cloudflared-darwin-arm64.tgz';
        return base + 'cloudflared-darwin-amd64.tgz';
    }
    return null;
}

async function downloadCloudflared(destPath) {
    return new Promise((resolve, reject) => {
        const url = getDownloadUrl();
        if (!url) {
            reject(new Error(`Unsupported platform: ${process.platform}/${process.arch}`));
            return;
        }

        if (adapter) adapter.log.info(`[Tunnel] Downloading cloudflared from GitHub...`);
        
        const file = fs.createWriteStream(destPath);
        
        function followRedirects(reqUrl, depth = 0) {
            if (depth > 10) { reject(new Error('Too many redirects')); return; }
            
            https.get(reqUrl, (response) => {
                if (response.statusCode === 301 || response.statusCode === 302 || response.statusCode === 307) {
                    followRedirects(response.headers.location, depth + 1);
                    return;
                }
                if (response.statusCode !== 200) {
                    reject(new Error(`HTTP ${response.statusCode}`));
                    return;
                }
                response.pipe(file);
                file.on('finish', () => {
                    file.close();
                    // Linux/Mac: Executable-Bit setzen
                    if (process.platform !== 'win32') {
                        try { fs.chmodSync(destPath, '755'); } catch(e) {}
                    }
                    resolve();
                });
            }).on('error', (e) => {
                fs.unlink(destPath, () => {});
                reject(e);
            });
        }
        
        followRedirects(url);
    });
}

async function ensureCloudflared(dataDir) {
    const cloudflaredPath = getCloudflaredPath(dataDir);
    
    if (fs.existsSync(cloudflaredPath)) {
        return cloudflaredPath;
    }
    
    if (!fs.existsSync(dataDir)) {
        fs.mkdirSync(dataDir, { recursive: true });
    }
    
    await downloadCloudflared(cloudflaredPath);
    return cloudflaredPath;
}

// ─── TUNNEL START ─────────────────────────────────────────────────────────────

function getAdapterDataDir(adapterRef) {
    // Verwende das ioBroker-Datenverzeichnis des Adapters (immer beschreibbar + ausführbar)
    try {
        const utils = require('@iobroker/adapter-core');
        const baseDir = utils.getAbsoluteDefaultDataDir();
        return path.join(baseDir, 'cogni-living');
    } catch(e) {
        // Fallback: Verzeichnis neben dem Adapter-Modul (auch ausführbar)
        return path.join(__dirname, '..', '..', 'data');
    }
}

async function start(adapterRef, port) {
    adapter = adapterRef;
    
    if (tunnelProcess) {
        adapterRef.log.warn('[Tunnel] Already running. Stop first.');
        return currentUrl;
    }

    const dataDir = getAdapterDataDir(adapterRef);
    
    try {
        const cloudflaredPath = await ensureCloudflared(dataDir);
        adapterRef.log.info(`[Tunnel] Starting Quick Tunnel → http://localhost:${port}`);
        
        return await startTunnel(cloudflaredPath, port, adapterRef);
    } catch(e) {
        adapterRef.log.error(`[Tunnel] Failed to start: ${e.message}`);
        return null;
    }
}

async function startTunnel(cloudflaredPath, port, adapterRef) {
    return new Promise((resolve) => {
        // Einfachster stabiler Start: nur --url, sonst nichts
        const args = ['tunnel', '--url', `http://localhost:${port}`];
        
        tunnelProcess = spawn(cloudflaredPath, args, {
            stdio: ['pipe', 'pipe', 'pipe'],
            env: { ...process.env, TERM: 'xterm' }
        });
        
        // stdin sofort schließen (cloudflared braucht es nicht)
        tunnelProcess.stdin.end();

        let resolved = false;
        let outputBuffer = '';

        const timeout = setTimeout(() => {
            if (!resolved) {
                adapterRef.log.warn('[Tunnel] Timeout waiting for URL (60s). Last output:\n' + outputBuffer.slice(-500));
                resolve(null);
                resolved = true;
            }
        }, 60000);

        // Verarbeite ALLE Output-Zeilen – nicht nur die URL
        function processOutput(data) {
            const text = data.toString();
            outputBuffer += text;
            // Limitiere Buffer auf 2KB
            if (outputBuffer.length > 2048) outputBuffer = outputBuffer.slice(-2048);

            // Debug-Log für alle Zeilen (nur debug-Level um Logs nicht zu spammen)
            for (const line of text.split('\n')) {
                const trimmed = line.trim();
                if (!trimmed) continue;
                // Wichtige Zeilen als info, Rest als debug
                if (trimmed.toLowerCase().includes('error') || trimmed.toLowerCase().includes('failed')) {
                    adapterRef.log.warn(`[Tunnel] ${trimmed}`);
                } else if (trimmed.includes('trycloudflare') || trimmed.includes('registered') || trimmed.includes('connected')) {
                    adapterRef.log.info(`[Tunnel] ${trimmed}`);
                } else {
                    adapterRef.log.debug(`[Tunnel] ${trimmed}`);
                }
            }

            // URL extrahieren
            const match = text.match(/https:\/\/[a-z0-9\-]+\.trycloudflare\.com/i);
            if (match && !resolved) {
                currentUrl = match[0];
                clearTimeout(timeout);
                resolved = true;
                
                adapterRef.log.info(`[Tunnel] ✅ Cloudflare URL aktiv: ${currentUrl}`);
                
                const fullUrl = currentUrl + '/?token=' + (adapterRef.config.familyShareToken || '');
                adapterRef.setStateAsync('system.pwaTunnelUrl', { val: fullUrl, ack: true }).catch(() => {});
                
                // Health-Check: Prüfe 5s nach URL ob Prozess noch lebt
                setTimeout(() => {
                    if (tunnelProcess && tunnelProcess.exitCode !== null) {
                        adapterRef.log.error(`[Tunnel] ⚠ Prozess ist 5s nach URL-Generierung gestorben! Letzte Ausgabe:\n${outputBuffer.slice(-400)}`);
                    } else if (tunnelProcess) {
                        adapterRef.log.info(`[Tunnel] ✅ Prozess läuft stabil (PID: ${tunnelProcess.pid})`);
                    }
                }, 5000);
                
                resolve(currentUrl);
            }
        }

        // Beide Streams kontinuierlich lesen (verhindert Pipe-Buffer-Blockierung)
        tunnelProcess.stdout.on('data', processOutput);
        tunnelProcess.stderr.on('data', processOutput);

        tunnelProcess.on('exit', (code, signal) => {
            adapterRef.log.warn(`[Tunnel] Process exited (code ${code}, signal ${signal})`);
            if (code !== 0) {
                adapterRef.log.warn(`[Tunnel] Last output: ${outputBuffer.slice(-300)}`);
            }
            tunnelProcess = null;
            currentUrl = null;
            
            if (!resolved) { resolve(null); resolved = true; }
            
            // Auto-Restart nach 15s (nur wenn Cloudflare noch aktiviert ist)
            if (adapterRef.config.cloudflareEnabled) {
                setTimeout(() => {
                    if (!tunnelProcess && adapter) {
                        adapterRef.log.info('[Tunnel] Auto-restarting...');
                        startTunnel(cloudflaredPath, port, adapterRef);
                    }
                }, 15000);
            }
        });

        tunnelProcess.on('error', (e) => {
            adapterRef.log.error(`[Tunnel] Spawn error: ${e.message}`);
            if (!resolved) { resolve(null); resolved = true; }
        });
    });
}

function stop() {
    if (tunnelProcess) {
        tunnelProcess.kill('SIGTERM');
        tunnelProcess = null;
        currentUrl = null;
        if (adapter) adapter.log.info('[Tunnel] Stopped.');
    }
}

function getUrl() {
    return currentUrl;
}

module.exports = { start, stop, getUrl };
