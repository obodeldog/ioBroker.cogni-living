...
function isRelevantActivity(type, value) {
    ...
    const isMotion = ['motion', 'bewegung', 'prÃ¤senz', 'presence', 'occupancy'].some(k => t.includes(k));
    const isDoor = ['door', 'tÃ¼r', 'window', 'fenster', 'griff', 'handle', 'lock', 'schloss', 'contact', 'kontakt'].some(k => t.includes(k));
...
async function processSensorEvent(adapter, id, state, deviceConf) {
    ...
    const eventObj = { timestamp: now, id: id, name: deviceConf.name, type: deviceConf.type, location: deviceConf.location, value: value,
        ...
        isFP2Bed:         (_typeLC === 'presence_radar' && _sf === 'bed')    || deviceConf.isFP2Bed    || false,
        isFP2Living:      (_typeLC === 'presence_radar' && _sf === 'living') || deviceConf.isFP2Living  || false,
        isVibrationBed:   (_typeLC === 'vibration' && _sf === 'bed')         || deviceConf.isVibrationBed || false,
