...
// Gilt für: motion, door, presence_radar_bool, vibration_trigger und alle boolean-artigen Sensoren
...
// Gibt Personenzahl für presence_radar_count zurück (FP2 .value State)
...
async function recordSequence(adapter, eventObj) {
    ...
    const validTypes = ['motion', 'bewegung', 'prÃ¤senz', 'presence', 'occupancy', 'door', 'tÃ¼r', 'window', 'fenster', 'contact', 'kontakt', 'light', 'licht', 'switch', 'schalter', 'dimmer', 'lampe'];
...
function isRelevantActivity(type, value) {
    ...
    // presence_radar_count: Personenzahl-Sensor – Aktivität wenn Personen > 0
    if (t === 'presence_radar_count') return toPersonCount(value) > 0;
    // vibration_strength: Stärke-Sensor – Aktivität wenn Wert > 0
    ...
    const isMotion = ['motion', 'presence_radar_bool', 'bewegung', 'praesenz', 'presence', 'occupancy'].some(k => t.includes(k));
    const isDoor = ['door', 'tur', 'window', 'fenster', 'griff', 'handle', 'lock', 'schloss', 'contact', 'kontakt'].some(k => t.includes(k));
...
async function processSensorEvent(adapter, id, state, deviceConf) {
    ...
    const eventObj = { timestamp: now, id: id, name: deviceConf.name, type: deviceConf.type, location: deviceConf.location, value: value,
        ...
        isFP2Bed:         ((_typeLC === 'presence_radar_bool' || _typeLC === 'presence_radar_count') && _sf === 'bed')    || deviceConf.isFP2Bed    || false,
        isFP2Living:      ((_typeLC === 'presence_radar_bool' || _typeLC === 'presence_radar_count') && _sf === 'living') || deviceConf.isFP2Living  || false,
        isVibrationBed:   ((_typeLC === 'vibration_trigger' || _typeLC === 'vibration_strength') && _sf === 'bed')        || deviceConf.isVibrationBed || false,
        isPersonCount:    _typeLC === 'presence_radar_count',
    ...
    if (adapter.isProVersion && adapter.sendToPythonWrapper) {
        ...
        const isDoorOrWindow = ['door', 'tÃ¼r', 'window', 'fenster', 'griff', 'handle', 'lock', 'schloss'].some(t => type.includes(t));
        const isLightOrSwitch = ['light', 'licht', 'switch', 'schalter', 'dimmer', 'lampe'].some(t => type.includes(t));
        const isMotion = ['motion', 'bewegung', 'prÃ¤senz', 'presence', 'occupancy'].some(t => type.includes(t));
