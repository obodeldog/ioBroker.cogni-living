...
async function scanThermostats(adapter) {
    const devices = adapter.config.devices || [];
    const results = [];
    const sensors = devices.filter(d => (d.id && d.id.includes('temperature')) || (d.type && ['temperature', 'thermostat', 'sensor'].includes(d.type)) || (d.name && d.name.toLowerCase().includes('temp')));
