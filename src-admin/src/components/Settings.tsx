...
interface ScannedDevice extends DeviceConfig { selected?: boolean; _score?: number; exists?: boolean; scanningMethod?: string; }
...
export default class Settings extends React.Component<SettingsProps, SettingsState> {
    ...
    handleImportDevices = () => { const selected = this.state.scannedDevices.filter(d => d.selected); if(selected.length === 0) { this.setState({ showWizard: false }); return; } const currentDevices = [...this.state.devices]; let addedCount = 0; selected.forEach(newItem => { if(!currentDevices.find(d => d.id === newItem.id)) { const { selected, _score, exists, scanningMethod, ...deviceConfig } = newItem; currentDevices.push(deviceConfig); addedCount++; } }); this.updateDevices(currentDevices); this.showSnackbar(`${addedCount} Sensoren importiert.`, 'success'); this.setState({ showWizard: false }); }
        ...
        return (
            <Box>
                <Box sx={{ mb: 2, display: 'flex', gap: 2 }}>
                    <Button variant="contained" startIcon={<AutoFixHighIcon />} onClick={this.handleOpenWizard} color="primary">Auto-Discovery (Wizard)</Button>
                    <Button variant="outlined" startIcon={<AddIcon />} onClick={() => this.onAddDevice()}>Manueller Sensor</Button>
                    <Button variant="outlined" startIcon={<AddIcon />} onClick={() => this.onAddPresenceDevice()}>Präsenz-Gerät</Button>
                    <Box sx={{ flexGrow: 1 }} />
                    <Button variant="text" color="error" startIcon={<DeleteIcon />} onClick={() => this.setState({ showDeleteConfirm: true })} disabled={this.state.devices.length === 0}>Alle Löschen</Button>
                </Box>
                        ...
                        <TableBody>
                            {this.state.devices.map((device, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell>
                                        <Select value={device.type} onChange={e => this.onDeviceChange(index, 'type', e.target.value)} size="small" variant="standard" sx={{minWidth: 120}}>
                                            {SENSOR_TYPES.map(t => <MenuItem key={t.id} value={t.id}>{t.label}</MenuItem>)}
                                        </Select>
                                    </TableCell>
                                    <TableCell><Checkbox checked={device.isExit} onChange={e => this.onDeviceChange(index, 'isExit', e.target.checked)} size="small"/></TableCell>
                                    ...
                                    <TableCell><IconButton onClick={() => this.onDeleteDevice(index)} size="small"><DeleteIcon /></IconButton></TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
