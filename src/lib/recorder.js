...
async function recordSequence(adapter, eventObj) {
    ...
    // Nur aktuelle Sensortypen aus der Sensorliste (kein Legacy)
    const validTypes = ['motion', 'presence_radar_bool', 'presence_radar_count', 'door', 'lock', 'blind', 'light', 'dimmer', 'vibration_trigger', 'vibration_strength'];
...
function isRelevantActivity(type, value) {
    ...
    const isMotion = t === 'motion' || t === 'bewegung' || t === 'praesenz' || t === 'presence'
                  || t === 'presence_radar_bool' || t === 'presence_radar_count' || t === 'occupancy';
...
async function processSensorEvent(adapter, id, state, deviceConf) {
    ...
    const isTemp = type === 'temperature' || type === 'thermostat';
    ...
    const eventObj = { timestamp: now, id: id, name: deviceConf.name, type: deviceConf.type, location: deviceConf.location, value: value,
        ...
        isFP2Bed:         ((_typeLC === 'presence_radar_bool' || _typeLC === 'presence_radar_count') && (_sf === 'bed' || _sf === 'bed_zone')) || deviceConf.isFP2Bed || false,
        ...
        isBedroomMotion:  (_typeLC === 'motion' && _sf === 'bed') || false,
        isBedroomNonBed:  ((_typeLC === 'presence_radar_bool' || _typeLC === 'presence_radar_count') && _sf === 'bedroom_nonbed') || false,
    ...
    if (adapter.isProVersion && adapter.sendToPythonWrapper) {
        ...
        const isDoorOrWindow = ['door', 'lock'].includes(type);
        const isLightOrSwitch = ['light', 'dimmer'].includes(type);
